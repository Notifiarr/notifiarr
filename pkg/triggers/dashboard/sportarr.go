...
func (c *Cmd) getSportarrStates(ctx context.Context) []*State {
	states := []*State{}

	for instance, app := range c.Apps.Sportarr {
		if !app.Enabled() || !c.Enabled.Sportarr {
			continue
		}

		mnd.Log.Debugf(mnd.GetID(ctx), "Getting Sportarr State: %d:%s", instance+1, app.URL)

		state, err := c.getSportarrState(ctx, instance+1, &app)
		if err != nil {
			state.Error = err.Error()
			mnd.Log.Errorf(mnd.GetID(ctx), "Getting Sportarr Queue from %d:%s: %v", instance+1, app.URL, err)
		}
...
func (c *Cmd) getSportarrState(ctx context.Context, instance int, app *apps.Sportarr) (*State, error) {
	...
	if state.Next, err = c.getSportarrStateUpcoming(app, state.Next); err != nil {
		return state, fmt.Errorf("instance %d: %w", instance, err)
	}

	if state.Latest, err = c.getSportarrHistory(app); err != nil {
		return state, fmt.Errorf("instance %d: %w", instance, err)
	}
...
func (c *Cmd) getSportarrHistory(app *apps.Sportarr) ([]*Sortable, error) {
...
func (c *Cmd) getSportarrStateUpcoming(app *apps.Sportarr, next []*Sortable) ([]*Sortable, error) {
