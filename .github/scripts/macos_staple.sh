#!/usr/bin/env bash
# Confirm Developer ID on Notifiarr.app, then staple the notarized DMG.
...
app="${root}/apps/notifiarr-app_darwinall/Notifiarr.app"
if [ ! -d "${app}" ]; then
  app=""
  while IFS= read -r p; do
    ...
    app="${p}"
    break
  done < <(find "${root}" -type d -name 'Notifiarr.app')
fi
if [ ! -d "${app}" ]; then
  while IFS= read -r p; do
    app="${p}"
...
if [ ! -d "${app}" ]; then
  echo "Notifiarr.app missing under ${root}" >&2
...
if ! grep -F "Developer ID Application" "${dump}" >/dev/null; then
  echo "Notifiarr.app is not Developer ID signed; macos_native skipped or failed" >&2
  cat "${dump}" >&2
  exit 1
fi
echo "signed ${app}"
