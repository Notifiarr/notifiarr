...
**AUR `notifiarr-bin`** (`init/archlinux/PKGBUILD.template`) downloads those same GitHub `.linux.gz` names. Keep **binary** AUR: a source PKGBUILD cannot `npm run build` without `FONTAWESOME_PACKAGE_TOKEN`.
