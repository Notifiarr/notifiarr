...
`REVISION` must be in the goreleaser-action `env:` map (Actions does not automatically forward `GITHUB_ENV` into a later step’s `env:` block). On `--nightly`, `nightly.version_template` already bakes it into `{{ .Version }}` (example `0.9.8-3273`), so man-page hooks use `{{ .Version }}` only — do not append `REVISION` again. The env var is still required: that template *creates* `.Version`, and tagged builds keep `.Version` as the semver while ldflags `Revision` / Darwin `CFBundleVersion` / Windows FileVersion still need the count. `golift.io/version.Version` is the `x.y.z` prefix of `{{ .Version }}` (not the full nightly string): the website and GUI concatenate Version + Revision. Darwin `CFBundleShortVersionString` and Windows FileVersion `x.y.z` use that same prefix, not `.RawVersion` (that stays on the last tag during `--nightly`).
